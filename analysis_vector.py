"""
Linkscape Corridor Analysis - Vector Workflow (v23.1)
----------------------------------------------------
...
def find_all_possible_corridors(
    ...
    Uses buffered straight-line connections to respect minimum width.
    """
...
def find_shortest_corridor_from_group(
    ...
    for candidate_id in candidate_ids:
        ...
        corridor_line = QgsGeometry.fromPolylineXY([QgsPointXY(p1), QgsPointXY(p2)])
        geom_candidate = _buffer_line_segment(corridor_line, params.min_corridor_width)
        geom_candidate = geom_candidate.difference(start_union)
        geom_candidate = geom_candidate.difference(target_geom)
        geom_candidate = geom_candidate.makeValid()
        if geom_candidate.isEmpty():
            continue
...
def run_vector_analysis(
    ...
    print("=" * 70)
    print("LINKSCAPE VECTOR ANALYSIS v23.1")
    print("=" * 70)
