...
## What's New in v1.4 (since v1.3)
- **Simpler strategy picker**: Only the two most dependable options—Most Connectivity and Largest Patch—ship in this build, so you can choose quickly and stay focused on planning.
- **Smarter raster runs**: Corridor searches look in every direction at once and Largest Patch adds growth in stages, helping you connect more habitat with the same budget.
- **Instant run summaries**: Each analysis now saves a plain-language text summary plus a small CSV you can load back into QGIS for filtering or sharing.
- **Friendlier guidance**: Progress updates and warnings are easier to read, and the documentation reflects this development build.
...
**Algorithm**: Linkscape v1.4  
**Platform**: QGIS 3.x  
**License**: [Specify license if applicable]

For questions, bug reports, or feature requests, contact [provide contact information or repository link].
